...
################# LOAD DATA ####################
# Soil temperature
temp19 <- read.csv("./clean_data/18-19_temp_data.csv", as.is=TRUE)
...
temp.s <- subset(temp,
                   datetime >= as.POSIXct('2018-10-03 00:00',
                                          tz = "MST") &
                     datetime <= as.POSIXct('2019-09-19 00:00',
                                            tz = "MST"))

############## A big overview plot ############
qplot(x=datetime, y=temp, 
      data=temp.s,
      main="2018-2019 Daily Air Temperature at RHF sites",
